...
const FAQS = [
  {
    q: 'AI Credits hoạt động như thế nào?',
    a: 'AI Credits là đơn vị dịch vụ nội bộ của PicSpy; dùng để mở khóa LensSpy AI và tải ảnh Premium. Mỗi gói trả phí nhận AI Credits định kỳ hàng tháng. AI Credits Free cấp 1 lần duy nhất, không reset.',
  },
  {
...
export default function PricingPage() {
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => {
        setPlans(data.plans)
        setFounderLeft(data.founderSlotsLeft)
      })
      .catch(() => setError(true))
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.message || 'Không thể nhận AI Credits')
    } finally {
      setClaiming(false)
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <Up>
          ...
          >
            ...
            <p
              className="text-base leading-relaxed max-w-xl mt-5"
              style={{ ...F.body, color: 'oklch(62% 0.01 285)' }}
            >
              Khám phá, copy và kiếm tiền từ prompt AI chất lượng cao. Creator
              Việt Nam đầu tiên được trả trực tiếp từ nền tảng.
            </p>
          </section>
        </Up>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p
                    ...
                    style={{ ...F.display, color: 'oklch(84% 0.14 65)' }}
                  >
                    Bạn chưa nhận 100 AI Credits khởi điểm
                  </p>
                  <p
                    className="text-xs mt-0.5"
                    style={{ ...F.body, color: 'oklch(70% 0.12 65)' }}
                  >
                    AI Credits dùng thử 1 lần.
                  </p>
                </div>
              </div>
              <motion.button
                ...
                style={{
                  ...
                  boxShadow:
                    'inset 0 1.5px 0 rgba(255,255,255,0.3), inset 0 -2px 0 rgba(0,0,0,0.15), 0 8px 24px rgba(217,119,6,0.45)',
                  color: W,
                  ...F.display,
                  minHeight: '44px',
                }}
        ...
        <Up delay={0.12} className="flex justify-center mb-10">
          ...
          >
            {Object.entries(CYCLES).map(([k, l]) => {
              ...
              const paidPlansForCycle = plans.filter(
                (p) => p.planId !== 'free' && Number(p.pricing?.[k] ?? 0) > 0
              ).length
                  ...
                  style={{
                    ...
                    ...(cycle === k
                      ? {
                          ...
                          boxShadow:
                            'inset 0 1.5px 0 rgba(255,255,255,0.25), inset 0 -1.5px 0 rgba(0,0,0,0.2), 0 4px 16px hsla(var(--color-brand-h), var(--color-brand-s), 50%, 0.35)',
                        }
                      : {
          ...
          {loading ? (
            ...
            (() => {
              ...
              const visiblePlans = plans.filter((plan) => {
                ...
                if (cycle === 'weekly')
                  return id !== 'free' && id !== 'founder' && price > 0
        ...
        <Up delay={0.55}>
          <section className="mt-20 pb-8">
            <div
              className="pt-14 border-t relative text-center"
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-12 mb-16">
            ...
            <div className="col-span-2">
              ...
              <p
                className="text-white/35 max-w-xs mb-7 leading-relaxed text-sm"
                style={F.body}
              >
              ...
              <div className="flex gap-3">
                {['🌐', '🎬', '📸'].map((icon, i) => (
                  <a
                    key={i}
                    href="#"
                    className="w-10 h-10 rounded-full flex items-center justify-center text-sm hover:scale-110 transition-transform"
                    style={glassCard}
                  >
                    {icon}
                  </a>
            ...
            {[
              {
                title: 'Khám phá',
                links: [
                  { label: 'Gallery cộng đồng', to: '/' },
                  { label: 'Bảng xếp hạng', to: '/search' },
                  { label: 'Bộ sưu tập mới', to: '/search' },
              ...
              },
              {
                ...
                links: [
                  ...
                  { label: 'Báo cáo vi phạm', to: '#' },
                  { label: 'Liên hệ', to: '#' },
                  { label: 'Blog sáng tạo', to: '#' },
                ],
              },
            ].map(({ title, links }) => (
              <div key={title}>
          ...
          <div className="pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p
              className="text-white/25 text-xs font-bold tracking-widest uppercase"
              style={F.body}
            >
              © 2026 PICSPY Vietnam. All rights reserved.
            </p>
            <p
              className="text-white/25 text-xs font-bold flex items-center gap-1"
              style={F.body}
            >
              Made with{' '}
