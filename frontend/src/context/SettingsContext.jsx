...
export function applyThemeBrandColors(
  ...
  if (shadowStyle === 'glow') {
    root.style.setProperty(
      ...
      `0 0 20px hsla(${h}, ${s}%, 55%, 0.35)`
    )
    root.style.setProperty(
      '--color-glass-hover-glow',
      `hsla(${h}, ${s}%, 55%, 0.25)`
    )
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => {
        ...
        if (data?.discoveryAutoScrollInterval !== undefined) {
          setDiscoveryAutoScrollInterval(data.discoveryAutoScrollInterval)
        }
        if (data?.discoveryAutoScrollStagger !== undefined) {
          setDiscoveryAutoScrollStagger(data.discoveryAutoScrollStagger)
        }
      })
      .catch((err) => {
