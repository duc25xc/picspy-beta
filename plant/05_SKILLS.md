# 🧠 PixelDrop — Skills & Technical Knowledge Guide
...
const imageWorker = new Worker('image-processing', async (job) => {
  ...
  try {
    ...
    const { data, info } = await sharp(imageBuffer)
      .resize(32, 32).raw().ensureAlpha().toBuffer({ resolveWithObject: true })
    ...
    const status = nsfwScore > 0.8 ? 'rejected' :
                   nsfwScore > 0.4 ? 'pending' : 'approved'
...
const updateScores = async () => {
  ...
  const topPosts = await Post.find({ status: 'approved' })
    .sort({ score: -1 }).limit(100).select('_id score')
...
const downloadPremiumPost = async (userId, postId) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (!post?.isPremium) throw new AppError('NOT_PREMIUM', 'Ảnh này miễn phí')
      ...
      await Transaction.insertMany([
        ...
        {
          userId: post.authorId, type: 'earn_download_premium',
          amount: creatorEarns,
          relatedPostId: postId,
      ...
      result = { downloadUrl, coinsSpent: required, creatorEarned: creatorEarns }
    })
...
const searchByColor = async (targetHex, limit = 20) => {
  ...
  const posts = await Post.find({ status: 'approved' })
    .select('_id colorPalette stats score')
